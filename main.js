...
function crearDivImportar() {
	...
	function importar() {
      ...
      if(regs){
        ...
        regs=importar.registros
      }
      if(favs){
        favs = favs.concat(importar.favoritos);
      }else{
        favs=importar.favoritos
      }
...
function crearDivRegistros() {
	...
	for (let i = 0; i < dias.length; i++) {
		...
		let dia = document.getElementsByClassName("dia");
			dia[0].style = "background-color:" + colores[diaObj.getDay()];
...
/* function cargar_php() {
	...
	fetch(url, opciones)
		...
		.then((data) => {
			console.log(data);
			if (comprobarJSON(texto.value)) {
				console.log(response);
				for (let i = 0; i < response.registros.length; i++) {
					response.registros[i].fecha = new Date(response.registros[i].fecha);
				}
				regs = regs.concat(response.registros);
				favs = favs.concat(response.favoritos);
...
function crearElemento(nombreVariable, padre, tipoElemento, texto, clases, funcionElemento, id) {
